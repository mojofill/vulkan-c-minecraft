...
static void createInstance(vk_context *vko) {
    ...
    createInfo.pApplicationInfo = &appInfo; uint32_t glfwExtensionCount;
...
static void createLogicalDevice(vk_context *vko) {
    const char* deviceExtensions[] = { VK_KHR_SWAPCHAIN_EXTENSION_NAME }; // must add swapchain extension
...
static void createUniformBuffers(vk_context *vko) {
    ...
    vko->uniformBuffers = malloc(MAX_FRAMES_IN_FLIGHT * sizeof(VkBuffer));
    vko->uniformBufferMemories = malloc(MAX_FRAMES_IN_FLIGHT * sizeof(VkDeviceMemory));
    vko->uniformBuffersMapped = malloc(MAX_FRAMES_IN_FLIGHT * bufferSize); // i think so?
...
static void initVulkan(vk_context *vko, VertexBufferContext *vbo) {
    ...
    createTextureImage(vko, "./src/king_trump_cropped.png");
    ...
    createUniformBuffers(vko); // actually populates data into uniform layouts
...
// camera work goes into updateUniformBuffers
static void updateCameraUniforms(vk_context *vko, uint32_t currentImage) {
    ...
    memcpy(vko->uniformBuffersMapped[currentImage], &ubo, sizeof(ubo));
}
...
static void drawFrame(vk_context *vko, uint32_t *currentFrame) {
    ...
    updateCameraUniforms(vko, *currentFrame);

    recordCommands(vko, *currentFrame);
...
static void cleanup(vk_context *vko) {
    ...
    free(vko->uniformBuffers);
    free(vko->uniformBufferMemories);
    free(vko->uniformBuffersMapped); // might not need this - actually yes i do, b/c this memory will be reused every frame
