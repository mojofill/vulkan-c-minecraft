...
#include "camera.h" // this some HACKYY SHIT!! I DONT LIKE THIS but wtv
...
typedef struct VertexBufferContext {
    ...
    VkVertexInputAttributeDescription attrDescs[2]; // for now only two attributes
    char *test;
} VertexBufferContext;
...
typedef struct vk_context {
    ...
    void* *uniformBuffersMapped; // create pointer instantly after gpu memory creation for a persistent pointer for use later
